module music_player(clk, reset, play_pause, next, NewFrame, sample, play, song);
    ...
    wire reset_play; // 主控制器输出
    wire song_done, new_note; // 乐曲读取输出信号
    wire[5:0] note, duration; // 音符标记及持续时间
    wire note_done, sample_ready; // 音符播放输出信号
    wire ready; // 同步化电路输出
    wire beat; // 节拍基准产生器输出
    ...
    // 乐曲读取song_reader
    song_reader song0(.clk(clk), .reset(reset_play), .play(play), .song(song), .note_done(note_done), .song_done(song_done), .note(note), .duration(duration), .new_note(new_note));
    ...
    // 同步化电路syn
    syn syn0(.in(NewFrame), .clk(clk), .out(ready));
