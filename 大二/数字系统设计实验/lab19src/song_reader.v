module song_reader(clk, reset, play, song, note_done, song_done, note, duration, new_note);
    ...
    wire[4:0] q; // 乐曲音符地址后5位
    wire co; // 地址计数器进位

    // 地址计数器
    counter_n #(.n(32), .counter_bits(5)) cnt0(.clk(clk), .en(note_done), .r(reset), .q(q), .co(co));

    // 只读存储器
    song_rom rom0(.clk(clk), .dout({note, duration}), .addr({song, q}));

    // 结束判断
    song_over over0(.clk(clk), .ci(co), .duration(duration), .song_done(song_done));
